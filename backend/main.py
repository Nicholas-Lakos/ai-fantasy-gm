...
async def espn(req,views=None,period=None,filter_body=None):
 ...
 async with httpx.AsyncClient(timeout=40,follow_redirects=True) as c: r=await c.get(url,params=params,cookies=cookies,headers=headers)
...
async def live(u):
 ...
 meta=await espn(req,['mSettings','mTeam','mStandings','mStatus'])
 period=current_period(meta)
 if period is None: raise HTTPException(502,'ESPN did not provide the current scoring period.')
 # ESPN baseball rosters are scoring-period scoped; this is the critical live-roster request.
 roster_data=await espn(req,['mTeam','mRoster','mStandings','mStatus'],period)
 roster_data['settings']=meta.get('settings') or roster_data.get('settings') or {}
 roster_data['scoringPeriodId']=period
 ...
 return l,req,roster_data,period

async def free_players(req,period):
 if period is None: return []
 try:
  d=await espn(req,['kona_player_info'],period,{'players':{'filterStatus':{'value':['FREEAGENT','WAIVERS']},'limit':200,'sortPercOwned':{'sortPriority':1,'sortAsc':False}}})
  out=[]
  for x in d.get('players',[]) or []:
   ...
   out.append({'id':pid,'name':p.get('fullName') or f'Player {pid}','position':POS.get(p.get('defaultPositionId'),'—'),'eligible_positions':[POS.get(v,'—') for v in (p.get('eligibleSlots') or [])],'injury_status':p.get('injuryStatus'),'total_points':pool.get('totalPoints'),'percent_owned':pool.get('percentOwned'),'percent_started':pool.get('percentStarted'),'rank':pool.get('rank'),'status':status,'pro_team_id':p.get('proTeamId')})
  return out
...
async def ai_call(system,prompt):
 ...
 models=[]
 for m in [OR_MODEL,'openrouter/free','nvidia/nemotron-3-nano-30b-a3b:free']:
 ...
 async with httpx.AsyncClient(timeout=75,follow_redirects=True) as c:
  for model in models:
   try:
    payload={'model':model,'messages':[{'role':'system','content':system},{'role':'user','content':prompt}],'max_tokens':1800,'temperature':0.2}
...
app=FastAPI(title='AI Fantasy GM',version='8.0'); app.add_middleware(CORSMiddleware,allow_origins=['*'],allow_methods=['*'],allow_headers=['*'])
@app.get('/health')
def health(): return {'ok':True,'app':'AI Fantasy GM','version':'8.0','ai_provider':'openrouter-free','ai_configured':bool(os.getenv('OPENROUTER_API_KEY'))}
...
async def dashboard(authorization:str=Header(None)):
 u=uid(authorization); l,req,d,period=await live(u); ss,rank=summary(d,l['team_id']); t=next((x for x in d.get('teams',[]) if x.get('id')==l['team_id']),None) or {}; return {'league':(d.get('settings') or {}).get('name') or l['league_name'],'rank':rank,'record':rec(t),'team':compact_team(t),'standings':ss,'teams':[compact_team(x) for x in d.get('teams',[]) or []],'status':d.get('status') or {},'scoring_period':period,'live_data':True}
@app.get('/league/teams')
async def teams(authorization:str=Header(None)):
 u=uid(authorization); l,req,d,period=await live(u); ss,rank=summary(d,l['team_id']); return {'league':(d.get('settings') or {}).get('name') or l['league_name'],'my_team_id':l['team_id'],'scoring_period':period,'standings':ss,'teams':[compact_team(x) for x in d.get('teams',[]) or []]}
@app.get('/league/teams/{team_id}')
async def team(team_id:int,authorization:str=Header(None)):
 u=uid(authorization); l,req,d,period=await live(u); t=next((x for x in d.get('teams',[]) if x.get('id')==team_id),None)
...
async def waivers(authorization:str=Header(None)):
 u=uid(authorization); l,req,d,period=await live(u); players=await free_players(req,period); players.sort(key=lambda x:(x.get('total_points') or 0),reverse=True); return {'scoring_period':period,'players':players,'count':len(players),'live_data':True}
@app.post('/ai/clear')
...
async def gm(q:Question,authorization:str=Header(None)):
 u=uid(authorization); l,req,d,period=await live(u); waivers=await free_players(req,period); ss,rank=summary(d,l['team_id'])
 teams=[compact_team(x) for x in d.get('teams',[]) or []]; my=next((x for x in teams if x['id']==l['team_id']),None); opponents=[x for x in teams if x['id']!=l['team_id']]
 ctx={'league':(d.get('settings') or {}).get('name'),'season':d.get('seasonId',l['season']),'scoring_period':period,'data_timestamp':datetime.utcnow().isoformat(),'my_team':my,'opponent_teams':opponents,'standings':ss,'scoring_settings':(d.get('settings') or {}).get('scoringSettings'),'roster_settings':(d.get('settings') or {}).get('rosterSettings'),'live_waivers_and_free_agents':waivers}
 c=db(); rows=c.execute('SELECT role,content FROM ai_messages WHERE user_id=? ORDER BY id DESC LIMIT 10',(u,)).fetchall(); history=list(reversed([dict(x) for x in rows]))
 system='You are AI Fantasy GM, an expert fantasy baseball manager. Answer ONLY the user’s fantasy baseball question. Never output safety classifications, moderation labels, policy text, or "User Safety". The LIVE ESPN DATA in the prompt is authoritative and was fetched for the current scoring period immediately before this request. You MUST use the actual MY TEAM roster and LIVE WAIVERS/Free Agents when relevant. You MUST use opponent rosters for trade and matchup questions. Never invent players or stats. If a requested player is not in the supplied data, say so. For waivers, rank real available players from the supplied pool and recommend adds/drops. For trades say ACCEPT, DECLINE, or COUNTER and name the players. For lineup questions respect actual roster eligibility and lineup status. Be concise but explain the key evidence.'
 prompt='LIVE ESPN DATA — DO NOT SUBSTITUTE GENERIC FANTASY KNOWLEDGE:\n'+json.dumps(ctx,separators=(',',':'))+'\nRECENT CHAT:\n'+json.dumps(history,separators=(',',':'))+'\nUSER QUESTION:\n'+q.question
 ...
 return {'answer':answer,'context':{'league':ctx['league'],'scoring_period':period,'rank':rank,'my_roster_players':len((my or {}).get('roster',[])),'opponent_teams':len(opponents),'live_waiver_players':len(waivers),'live_data':True,'model':model,'timestamp':now}}
@app.get('/')
