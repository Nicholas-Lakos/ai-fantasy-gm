...
ESPN_BASE='https://lm-api-reads.fantasy.espn.com/apis/v3/games/flb/seasons';OR_BASE='https://openrouter.ai/api/v1/chat/completions';OR_MODEL=os.getenv('OPENROUTER_MODEL','openai/gpt-oss-20b:free')
...
async def ai_call(system,prompt):
 ...
 models=[]
 for m in [os.getenv('OPENROUTER_MODEL','openai/gpt-oss-20b:free'),'openrouter/free','nvidia/nemotron-3-nano-30b-a3b:free']:
 ...
 async with httpx.AsyncClient(timeout=75,follow_redirects=True) as c:
  for model in models:
   try:
    ...
    last='The selected model returned a safety classification instead of an answer.'
   except Exception as e:last=str(e)
 raise HTTPException(502,'AI service error: '+last)
app=FastAPI(title='AI Fantasy GM',version='7.0');app.add_middleware(CORSMiddleware,allow_origins=['*'],allow_methods=['*'],allow_headers=['*'])
@app.get('/health')
def health():return {'ok':True,'app':'AI Fantasy GM','version':'7.0','ai_provider':'openrouter-free','ai_configured':bool(os.getenv('OPENROUTER_API_KEY'))}
...
async def waivers(authorization:str=Header(None)):
 u=uid(authorization);l,req,d=await live(u);period=d.get('scoringPeriodId');players=await free_players(req,period);players.sort(key=lambda x:((x.get('total_points') or 0),-(x.get('percent_owned') or 0)),reverse=True);return {'scoring_period':period,'players':players,'count':len(players),'live_data':True}
@app.post('/ai/clear')
...
async def gm(q:Question,authorization:str=Header(None)):
 u=uid(authorization);l,req,d=await live(u);free=await free_players(req,d.get('scoringPeriodId'));ss,rank=summary(d,l['team_id']);ctx={'league':(d.get('settings') or {}).get('name'),'season':d.get('seasonId',l['season']),'my_team_id':l['team_id'],'my_rank':rank,'standings':ss,'teams':[compact_team(x) for x in d.get('teams',[]) or []],'scoring_settings':(d.get('settings') or {}).get('scoringSettings'),'roster_settings':(d.get('settings') or {}).get('rosterSettings'),'waivers':free}
 c=db();rows=c.execute('SELECT role,content FROM ai_messages WHERE user_id=? ORDER BY id DESC LIMIT 10',(u,)).fetchall();history=list(reversed([dict(x) for x in rows]));system='You are AI Fantasy GM, an expert fantasy baseball manager. You are NOT a safety classifier. Never answer with a safety label or the words User Safety. Answer the baseball question directly. ESPN data supplied below is the source of truth. Never invent players, stats, injuries, positions, scoring rules, or transactions. Give direct actionable advice. For trades explicitly say ACCEPT, DECLINE, or COUNTER and explain roster impact. For waiver questions, rank the best available adds, identify who to drop if appropriate, and use the live WAIVERS/FREEAGENT pool below. For lineup questions use eligibility and status. You are advisory only and cannot execute transactions.';prompt='LIVE ESPN DATA:\n'+json.dumps(ctx,separators=(',',':'))+'\nRECENT CHAT:\n'+json.dumps(history,separators=(',',':'))+'\nUSER QUESTION:\n'+q.question
