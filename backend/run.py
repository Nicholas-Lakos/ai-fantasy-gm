...
def compact_player_fixed(entry):
    ...
    return {
        ...
        'eligible_positions': [POS[x] for x in (p.get('eligibleSlots') or []) if x in POS],
...
async def pool_fixed(req, p, limit=500):
    filters = {'players': {'filterStatus': {'value': ['FREEAGENT', 'WAIVERS']}, 'limit': limit,
    ...
    if not items:
        try:
            data = await main.espn(req, ['kona_player_info'], p, {'players': {'limit': limit,
                              'sortPercOwned': {'sortPriority': 1, 'sortAsc': False}}}, timeout=35)
            items = data.get('players') or []
        except Exception:
            items = []
    out = {}
    for entry in items:
        ...
        if not pid:
            continue
        out[pid] = {'id': pid, 'name': p_obj.get('fullName') or f'Player {pid}',
                    'position': POS.get(p_obj.get('defaultPositionId'), '—'),
                    'eligible_positions': [POS[x] for x in (p_obj.get('eligibleSlots') or []) if x in POS],
                    'injury_status': p_obj.get('injuryStatus'),
                    'total_points': ppe.get('totalPoints', entry.get('totalPoints')),
                    'percent_owned': ppe.get('percentOwned', entry.get('percentOwned')),
                    'percent_started': ppe.get('percentStarted', entry.get('percentStarted')),
                    'rank': ppe.get('rank', entry.get('rank')), 'status': status,
                    'pro_team_id': p_obj.get('proTeamId')}
    return list(out.values())


async def player_card_fixed(req, pid, p):
    filters = {'players': {'filterIds': {'value': [int(pid)]},
    ...
    try:
        ...
        data = await main.espn(req, ['kona_player_info'], p,
                                {'players': {'filterIds': {'value': [int(pid)]}, 'limit': 1}}, timeout=30)
    items = data.get('players') or []
    if not items:
    ...
    for s in raw_stats:
        ...
        row = {'id': s.get('id'), 'seasonId': s.get('seasonId'),
               'scoringPeriodId': s.get('scoringPeriodId'), 'statTypeId': stid,
               'appliedTotal': total, 'appliedStats': applied}
        actual = code == f'00{req.season}' or (s.get('seasonId') == req.season and stid in (0, '0'))
        projection = code == f'10{req.season}' or (s.get('seasonId') == req.season and stid in (1, '1'))
        if actual:
            season_rows.append(row)
            for k, v in applied.items():
                display_stats[str(k)] = v
        elif projection:
            projection_rows.append(row)
        if s.get('scoringPeriodId') == p and total is not None:
            try: current_total = float(total)
    ...
    if not season_rows:
        ...
        for s in season_rows:
            for k, v in (s.get('appliedStats') or s.get('stats') or {}).items():
                display_stats[str(k)] = v
    position_id = p_obj.get('defaultPositionId')
    return {'id': pid, 'name': p_obj.get('fullName') or f'Player {pid}',
            'position': POS.get(position_id, '—'),
            'eligible_positions': [POS[x] for x in (p_obj.get('eligibleSlots') or []) if x in POS],
            'pro_team_id': p_obj.get('proTeamId'), 'injury_status': p_obj.get('injuryStatus'),
            'active': p_obj.get('active'), 'total_points': ppe.get('totalPoints', x.get('totalPoints')),
            'current_period_points': current_total, 'percent_owned': ppe.get('percentOwned', x.get('percentOwned')),
            'percent_started': ppe.get('percentStarted', x.get('percentStarted')),
            'stats': display_stats, 'season_splits': season_rows,
            'projection_stats': projection_rows, 'raw_stat_count': len(raw_stats)}
